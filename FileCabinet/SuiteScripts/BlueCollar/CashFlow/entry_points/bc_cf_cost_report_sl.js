...
define([
    ...
    /**
     * Build a stacked bar chart SVG for cost data.
     * Bars extend downward (costs are outflows).
     */
    const buildCostChart = (groups, periods, totals) => {
        ...
        const topBaseline = topMargin;

        let svg = '';

        // Grid lines at 25%, 50%, 75%
        [0.25, 0.5, 0.75].forEach((pct) => {
            const y = topBaseline + Math.round(barAreaHeight * pct);
            svg += '<line x1="' + padding + '" y1="' + y + '" x2="' + (vbWidth - padding) + '" y2="' + y + '" stroke="#E5E7EB" stroke-width="0.5"/>';
        });

        // Top baseline
        svg += '<line x1="' + (padding - 10) + '" y1="' + topBaseline + '" x2="' + (vbWidth - padding + 10) + '" y2="' + topBaseline + '" stroke="#6B7280" stroke-width="1" stroke-dasharray="6,4"/>';
        svg += '<text x="' + (padding - 14) + '" y="' + (topBaseline + 4) + '" text-anchor="end" fill="#6B7280" font-size="10" font-weight="500" font-family="Inter,sans-serif">$0</text>';
        ...
        periods.forEach((p, i) => {
            ...
            // Stacked bars downward
            let barY = topBaseline;
            groupNames.forEach((g, gi) => {
                const amt = (groups[g][p] || 0);
                if (amt <= 0) return;
                const h = Math.max(Math.round((amt / maxTotal) * barAreaHeight), 2);
                svg += '<rect x="' + x + '" y="' + barY + '" width="' + barWidth + '" height="' + h + '" rx="3" fill="' + costColors[gi % costColors.length] + '" opacity="0.92"/>';
                barY += h;
            });

            // Total label below bars
            if (total > 0) {
                svg += '<text x="' + cx + '" y="' + (barY + 14) + '" text-anchor="middle" fill="#04233D" font-size="10" font-weight="600" font-family="Inter,sans-serif">' + fmtCompact(total) + '</text>';
            }
        ...
        let legendX = padding;
        groupNames.forEach((g, gi) => {
            if (gi >= costColors.length) return;
            svg += '<rect x="' + legendX + '" y="' + (legendY - 8) + '" width="10" height="10" rx="2" fill="' + costColors[gi % costColors.length] + '"/>';
            legendX += 14;
        ...
        return '<div style="border:1px solid #D1D5DB;border-radius:8px;padding:20px;margin:16px 0;">'
            + '<div style="font-size:13px;font-weight:600;color:#6B7280;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:12px;">Cost by Month</div>'
