 ...
 * @description Cost Outflows Report — shell-only Suitelet.
 *              Returns an HTML skeleton immediately; client JS fetches from
 *              bc_cf_data_sl (?action=cost) and swaps in real content on resolve.
 *
 * URL Params (passed through to data SL via data-data-url):
 *   projectId – internal ID of the BC Project
 *   mode      – 'cash' (default) | 'accrual'
 *
 * Script ID:  customscript_bc_cf_cost_report_sl
 * Deploy  ID: customdeploy_bc_cf_cost_report_sl
 *
 * Spec: §3.4 (layout), §3.5 (no in-iframe nav tabs), §3.6 (KPIs — cost only),
 *       §3.7 (slate color encoding), §3.8 (single-color chart), §3.16 (skeletons)
 *
...
define([
    ...
    const buildSkeletonKpis = () => {
        const labels = ['Total Cost', 'Current Month', 'Peak Month', 'Remaining'];
        let cards = '';
    ...
    // Double-evaluation guard: if (window.__bccfWiredCost) return;
    //
    /* eslint-disable */
    const CLIENT_SCRIPT = `
(function () {
    ...
    if (window.__bccfWiredCost) return;
    window.__bccfWiredCost = true;
    ...
    /**
     * Build 4 KPI cards for cost-only view:
     *   Total Cost     — accent class, slate value (--bccf-ink-500) per §3.7
     *   Current Month  — default ink, shows current month amount
     *   Peak Month     — default ink, shows highest month amount
     *   Remaining      — default ink, cost from current month to end
     *
    ...
    function renderKpis(kpis) {
        ...
        var cards = [
            ...
            // 2. Current Month — default ink
            '<div class="bccf-kpi">'
                + '<div class="bccf-k">Current Month</div>'
                + '<div class="bccf-v">' + esc(fmtCurrency(currentMonth)) + '</div>'
                + '<div class="bccf-sub">current month &middot; ' + esc(currentPct) + ' of total</div>'
            + '</div>',

            // 3. Peak Month — default ink
            '<div class="bccf-kpi">'
                + '<div class="bccf-k">Peak Month</div>'
                + '<div class="bccf-v">' + esc(fmtCurrency(peakMonth)) + '</div>'
                + '<div class="bccf-sub">highest month &middot; ' + esc(peakPct) + ' of total</div>'
            + '</div>',

            // 4. Remaining — default ink
            '<div class="bccf-kpi">'
                + '<div class="bccf-k">Remaining</div>'
                + '<div class="bccf-v">' + esc(fmtCurrency(remaining)) + '</div>'
                + '<div class="bccf-sub">' + esc(remPct) + ' of total</div>'
            + '</div>'
        ];
    ...
    /**
     * Single bar per month, slate color (--bccf-ink-500), using .bccf-bar.cost class.
    ...
    function renderChart(periods, categories) {
        ...
        var maxAmt = costTotal.reduce(function(m, v) { return Math.max(m, v); }, 1);
        ...
        var cols = periods.map(function(label, i) {
            ...
            return '<div class="' + (isNow ? 'now' : '') + '" style="display:flex;flex-direction:column;align-items:center;flex:1;min-width:48px;' + haloStyle + '">'
                ...
                + '<div style="font-size:11px;font-weight:600;color:var(--bccf-ink-700);margin-bottom:4px;white-space:nowrap;font-variant-numeric:tabular-nums">'
                    + esc(amtLabel)
                + '</div>'
                // Single bar — uses .bccf-bar.cost for slate color from bc_cf_styles
                + '<div style="display:flex;align-items:flex-end;height:' + BAR_MAX_H + 'px">'
                    + '<div class="bccf-bar cost" title="Cost: ' + esc(fmtCurrency(cost)) + '" style="width:24px;height:' + h + 'px;border-radius:3px 3px 0 0"></div>'
                + '</div>'
                // Month label below
        ...
        var headerHtml = '<span style="font-size:var(--bccf-text-base);font-weight:600;color:var(--bccf-ink-900)">Monthly Cost Outflow</span>';
     ...
     *   tbody: "Cost Forecast" category row (bold, slate) + sub-rows per line (indented)
     *   tfoot: Grand Total row — no Net row (cost-only report)
    ...
    function renderTable(periods, categories) {
        ...
        var totalCells = cost.total.map(function(v) {
        ...
        var tfoot = '<tfoot><tr>'
            ...
            + totalCells
            + '<td style="padding:8px 12px;text-align:right;font-size:var(--bccf-text-sm);font-weight:700;color:var(--bccf-ink-900);border-top:2px solid var(--bccf-border);font-variant-numeric:tabular-nums">' + esc(fmtCurrency(cost.grandTotal)) + '</td>'
        + '</tr></tfoot>';
            ...
            response.write(
                `<!doctype html><html><body style="font-family:sans-serif;padding:40px">
                    <div class="bccf-error-card" style="border-left:4px solid #c2361d;padding:14px 16px">
                        <h3 style="color:#c2361d;margin:0 0 8px">Cost Outflows Report Error</h3>
